...
data = pd.read_excel('UCD_trafo_monitors.xlsx', index_col=0)

print(data)
Test_dir = "C:/ucd_model_jh_mar_20/Network_data/Network_model/"
file_dir = Test_dir + "Feeder1/"
open(file_dir+'OpenDSS_Trafo_Monitors.txt', 'w').close()
...
for i in data.index:
    ...
    if (data.loc[i, 'Feeder']==1).all():
        string = 'New Monitor.' + i + ' Transformer.' + str(data.loc[i, 'Element']) + ' Terminal=' + str(data.loc[i, 'Terminal']) + ' Mode=' + str(data.loc[i, 'Mode'])
        file_dir = Test_dir + "Feeder1/"
        with open(file_dir+'OpenDSS_Trafo_Monitors.txt', 'a') as open_file:
